package org.firstinspires.ftc.teamcode.modernRobotics.gyro;
...
public class MRI_Gyro_Vid5_Straight extends LinearOpMode {  //Linear op mode is being used so the program does not get stuck in loop()
    ...
    public void runOpMode() {
        ...
        MLeft.setDirection(DcMotor.Direction.REVERSE);  //This robot has two gears between motors and wheels. If your robot does not, you will need to reverse only the opposite motor
    ...
    public void driveStraight(int duration, double power) {
        ...
        while (MLeft.getCurrentPosition() < duration + startPosition && opModeIsActive()) {  //While we have not passed out intended distance
            ...
            leftSpeed = Range.clip(leftSpeed, -0.2, 0.2);
            rightSpeed = Range.clip(rightSpeed, -0.2, 0.2);
