...
@Autonomous(name = "Blue BEACON", group = "10841 blue")
//@Disabled
public class AutonomousBlue_LaunchAndPressBeacons extends LinearOpMode {
...
//        }
        ...
        if (opModeIsActive())
            swingTurnRight(27, turnSpeed);

        /*
         * now drive forward to launch position
         */
        if (opModeIsActive())
            driveForDistanceInches(27,driveSpeed);
...
//            LaunchParticle(false);
        ...
        if (opModeIsActive())
            driveForDistanceInches(14,driveSpeed);
        ...
        if (opModeIsActive()) {
            LocateWhiteLine(10,"blue");
            driveForDistanceInches(1.75, 1.0);
        }

        if (opModeIsActive())
            swingTurnRight(59,turnSpeed);
        ...
        robot.firstBeaconSuccess = areBothSidesOfBeaconBlue();
        telemetry.addData("first beacon success? ", robot.firstBeaconSuccess);
        telemetry.update();

        if (opModeIsActive())
            driveForDistanceInches(-16, -driveSpeed); // back away from the beacon

        if (opModeIsActive())
            swingTurnLeft(91,turnSpeed);
        ...
        if (opModeIsActive()) {
            LocateWhiteLine(10,"blue");
//            driveForDistanceInches(-1, -driveSpeed);
        }

        if (opModeIsActive())
            swingTurnRight(90,turnSpeed);
